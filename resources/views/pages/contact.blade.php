    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-10">
        ...
        <div class="lg:col-span-2">
            <div class="text-center mb-6">
                <h2 class="text-xl md:text-2xl font-bold text-[#611a1d]">Artikel Hukum Terbaru</h2>
                <p class="text-[#8c2d33] text-sm md:text-base">Akses mudah kumpulan artikel hukum</p>
                <div class="space-y-6">
                    @foreach ($articles->take(5) as $article)
                    <div class="flex flex-col md:flex-row items-start bg-[#f6f1ed] p-6 rounded-lg shadow-md hover:shadow-lg transition duration-200">
                        <!-- Gambar Artikel -->
                        <div class="relative w-full md:w-20 h-20">
                            ...
                            <!-- Label Kategori -->
                            <span class="absolute top-0 left-0 bg-purple-500 text-white text-xs font-semibold px-2 py-1 rounded-tr-lg rounded-bl-lg">
                                {{ $article->category->name }}
                            </span>
                        </div>
                        <!-- Konten Artikel -->
                        <div class="flex-1 ml-0 md:ml-4">
                            <a href="{{ route('articles.show', $article->slug) }}" class="text-sm md:text-lg font-semibold text-[#611a1d] hover:text-[#8c2d33] hover:underline">
                                {{ $article->title }}
                            </a>
                            <div class="text-xs md:text-sm text-[#8c2d33] mt-1 flex flex-wrap gap-2">
        ...
        <aside class="lg:col-span-1">
            <div class="bg-[#f6f1ed] p-6 rounded-lg shadow-md">
                <h2 class="text-lg md:text-xl font-bold text-[#611a1d] border-l-4 border-[#8c2d33] pl-3 mb-4">Artikel Terpopuler</h2>
                <ul class="space-y-4">
